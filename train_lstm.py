...
class LSTM_variable_input(torch.nn.Module) :
    ...
    def forward(self, x, s):
        x = self.embeddings(x)
        x = self.dropout(x)
        x_pack = pack_padded_sequence(x, s.to(torch.device("cpu")), batch_first=True, enforce_sorted=False)
        out_pack, (ht, ct) = self.lstm(x_pack)
        out = self.linear(ht[-1])
        return out


model = LSTM_variable_input(len(vocab2index.keys()), 256, 300).to(device)
...
with open("accs/" + filename + "val_lstm.txt", "w") as f:
    f.write("[" + ",".join(val_accs_str) + "]")
with open("accs/" + filename + "train_lstm.txt", "w") as f:
